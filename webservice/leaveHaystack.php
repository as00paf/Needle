...
if (!empty($_POST)) {
	...
	if (!is_set($_POST['haystackId']) || !is_set($_POST['userId'])) {
		$response["success"] = 0;
    ...
    try {
        ...
        $response["success"] = 0;
        $response["message"] = "Database Error. User with id : ".$_POST['userId']." is not in haystack. Exception :" .$ex ;
		die(json_encode($response));
    }
	...
	if ($row == "" || $row == " ") {
		$response["success"] = 0;
        $response["message"] = "Error! User with id : ".$_POST['userId']." not in haystack.";
		die(json_encode($response));
    }else{
		//DELETE
		//New query
		$query = "DELETE FROM haystack_users WHERE userId = :userId AND haystackId = :haystackId";
...
?>
		...
		<form action="leaveHaystack.php" method="post"> 
		    User Id:<br /> 
		    <input type="text" name="userId" placeholder="0" /> 
		    <br /><br /> 
		    Haystack Id:<br /> 
